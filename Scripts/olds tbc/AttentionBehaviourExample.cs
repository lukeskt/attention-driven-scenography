...
namespace AttentionDrivenScenography
{
    public class AttentionBehaviourExample : AttentionBehaviour
    {
        ...
        public override void CurrentAttentionBehaviour()
        {
            ...
            var threshChecks = AttentionProcessors.ThresholdCheck((float)CurrentAttentionRating, threshes);
            foreach (var item in threshChecks)
            {
                if (item.Value == true)
                {
                    print($"Attention Rating {CurrentAttentionRating} is higher than {item.Key}!");
                    break;
                }
                else
                {
